...
void fsOpenUrlInBrowser (LPCTSTR pszUrl)
{
	...
	if (_tcsnicmp (pszUrl, _T("http"), 4) == 0)//大小写不敏感
		_tcscpy (szReg, _T("http"));
	else if (_tcsnicmp(pszUrl, _T("http"), 5) == 0)
	...
	if (ERROR_SUCCESS != RegOpenKeyEx (HKEY_CLASSES_ROOT, szReg, 0, KEY_QUERY_VALUE, &hReg))//找到默认程序
		goto _lErr;

	if (ERROR_SUCCESS != RegQueryValueEx (hReg, NULL, NULL, NULL, (LPBYTE) szBrowser, &dwBrowserLen))//得到键值
	{
		RegCloseKey (hReg);
		goto _lErr;
	}
	...
	if (32 >= (int)ShellExecute (HWND_DESKTOP, _T("open"), szBrowser [0] == '"' ? szBrowser+1 : szBrowser, pszUrl, NULL, SW_SHOW))
		goto _lErr;
...
_lErr:
	if (32 >= (int)ShellExecute(HWND_DESKTOP, _T("open"), pszUrl, NULL, NULL, SW_SHOW))
		...
		MessageBox(NULL, pszUrl, _T("打开链接错误"), MB_ICONERROR);
}
